ALL = """
    WITH tt AS (
        ...
        SELECT brand_name, store_code, member_newold_type, member_level_type, cast(sum(sales_income) AS DECIMAL(18, 3)) AS sales_income
        FROM ads_crm.member_analyse_fold_daily_income_detail
        ...
        GROUP BY brand_name, store_code, member_newold_type, member_level_type
    )
    SELECT DISTINCT
    ...
    GROUP BY f.brand_name, f.store_code, f.member_newold_type, f.member_level_type, tt.sales_income, lyst.sales_income
"""
...
DAILY = """
    WITH tt AS (
        ...
        AND date <= date(date('{end_date}') - interval '1' year)
        AND date >= date(date('{start_date}') - interval '1' year)
        GROUP BY brand_name, store_code, member_newold_type, member_level_type, date
    )
    SELECT DISTINCT
        f.brand_name AS brand,
        f.store_code AS zone,
    ...
    AND f.date <= date('{end_date}')
    AND f.date >= date('{start_date}')
    GROUP BY f.brand_name, f.store_code, f.member_newold_type, f.member_level_type, tt.sales_income, lyst.sales_income, f.date
"""
...
MONTHLY = """
    WITH tt AS (
        ...
        AND date <= date(date('{end_date}') - interval '1' year)
        AND date >= date(date('{start_date}') - interval '1' year)
        GROUP BY brand_name, store_code, member_newold_type, member_level_type, year(date), month(date)
    )
    SELECT DISTINCT
        f.brand_name AS brand,
        f.store_code AS zone,
    ...
    AND f.date <= date('{end_date}')
    AND f.date >= date('{start_date}')
    GROUP BY f.brand_name, f.store_code, f.member_newold_type, f.member_level_type, tt.sales_income, lyst.sales_income, year(f.date), month(f.date)
"""
