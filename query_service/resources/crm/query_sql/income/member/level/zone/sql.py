...
DAILY = """
    WITH tt AS (
        ...
        SELECT brand_name, {zone}, member_level_type, cast(sum(sales_income) AS DECIMAL(18, 3)) AS sales_income, date
        FROM ads_crm.member_analyse_fold_daily_income_detail
        WHERE member_level_type IS NOT NULL AND member_type IS NULL AND member_newold_type IS NULL
        AND brand_name IN ({brands})
        AND {zone} IN ({zones})
...
MONTHLY = """
    ...
    AND f.date <= date('{end_date}')
    AND f.date >= date('{start_date}')
    GROUP BY f.brand_name, f.{zone}, f.member_level_type, tt.sales_income, lyst.sales_income,  year(f.date), month(f.date)
"""
