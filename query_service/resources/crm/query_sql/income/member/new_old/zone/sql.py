ALL = """
    WITH tt AS (
        ...
        SELECT ss.brand_name, ss.{zone}, ss.member_newold_type,
        ...
        LEFT JOIN lyst ON ss.brand_name = lyst.brand_name AND ss.{zone} = lyst.{zone} AND ss.member_newold_type = lyst.member_newold_type
        WHERE ss.member_newold_type IS NOT NULL AND ss.member_type IS NULL AND ss.member_level_type IS NULL
        ...
        GROUP BY ss.brand_name, ss.{zone}, ss.member_newold_type, lyst.store_array
    ), ss_lyst AS (
        SELECT ss_l.brand_name, ss_l.{zone}, ss_l.member_newold_type,
        cast(sum(ss_l.sales_income) AS DECIMAL(18, 3)) AS sales_income
        FROM ads_crm.member_analyse_fold_daily_income_detail ss_l
        LEFT JOIN ss ON ss_l.brand_name = ss.brand_name AND ss_l.{zone} = ss.{zone} AND ss_l.member_newold_type = ss.member_newold_type
        WHERE ss_l.member_newold_type IS NOT NULL AND ss_l.member_type IS NULL AND ss_l.member_level_type IS NULL
        ...
        GROUP BY ss_l.brand_name, ss_l.{zone}, ss_l.member_newold_type
    ), ss_now AS (
        SELECT DISTINCT
            f.brand_name,
            f.{zone},
        ...
        LEFT JOIN ss_lyst ON f.brand_name = ss_lyst.brand_name AND f.{zone} = ss_lyst.{zone} AND f.member_newold_type = ss_lyst.member_newold_type
        LEFT JOIN ss ON f.brand_name = ss.brand_name AND f.{zone} = ss.{zone} AND f.member_newold_type = ss.member_newold_type
        ...
        GROUP BY f.brand_name, f.{zone}, f.member_newold_type, ss_lyst.sales_income
    )
    SELECT DISTINCT
    ...
    LEFT JOIN ss_now ON f.brand_name = ss_now.brand_name AND f.{zone} = ss_now.{zone} AND f.member_newold_type = ss_now.member_newold_type
    WHERE f.member_newold_type IS NOT NULL AND f.member_type IS NULL AND f.member_level_type IS NULL
    AND f.brand_name IN ({brands})
    AND f.{zone} IN ({zones})
    AND f.order_channel IN ({order_channels})
    ...
    AND f.date <= date('{end_date}')
    AND f.date >= date('{start_date}')
    GROUP BY f.brand_name, f.{zone}, f.member_newold_type, tt.sales_income, lyst.sales_income, ss_now.compared_with_ss_lyst
"""
...
DAILY = """
    WITH l AS (
        ...
        SELECT brand_name, {zone}, member_newold_type, date,
        cast(sum(sales_income) AS DECIMAL(18, 3)) AS sales_income,
        array_distinct(array_agg(store_code)) AS store_array
        FROM ads_crm.member_analyse_fold_daily_income_detail
        WHERE member_newold_type IS NOT NULL AND member_type IS NULL AND member_level_type IS NULL
        ...
        SELECT ss.brand_name, ss.{zone}, ss.member_newold_type, ss.date,
        array_intersect(array_distinct(array_agg(ss.store_code)), lyst.store_array) AS store_array
        FROM ads_crm.member_analyse_fold_daily_income_detail ss
        LEFT JOIN lyst ON ss.brand_name = lyst.brand_name AND ss.{zone} = lyst.{zone} AND ss.member_newold_type = lyst.member_newold_type
        AND ss.date - interval '1' year = lyst.date
        WHERE ss.member_type IS NULL AND ss.member_newold_type IS NOT NULL AND ss.member_level_type IS NULL
        AND ss.brand_name IN ({brands})
        AND ss.{zone} IN ({zones})
        AND ss.order_channel IN ({order_channels})
        AND ss.sales_mode IN ({sales_modes})
        ...
        AND ss.date <= date('{end_date}')
        AND ss.date >= date('{start_date}')
        GROUP BY ss.brand_name, ss.{zone}, ss.member_newold_type, lyst.store_array, ss.date
    ), ss_lyst AS (
        SELECT ss_l.brand_name, ss_l.{zone}, ss_l.member_newold_type, ss_l.date,
        cast(sum(ss_l.sales_income) AS DECIMAL(18, 3)) AS sales_income
        FROM ads_crm.member_analyse_fold_daily_income_detail ss_l
        LEFT JOIN ss ON ss_l.brand_name = ss.brand_name AND ss_l.{zone} = ss.{zone} AND ss_l.member_newold_type = ss.member_newold_type
        AND ss.date - interval '1' year = ss_l.date
        WHERE ss_l.member_type IS NULL AND ss_l.member_newold_type IS NOT NULL AND ss_l.member_level_type IS NULL
        AND contains(ss.store_array, ss_l.store_code)
        AND ss_l.brand_name IN ({brands})
        AND ss_l.{zone} IN ({zones})
        AND ss_l.order_channel IN ({order_channels})
        ...
        AND ss_l.date <= date(date('{end_date}') - interval '1' year)
        AND ss_l.date >= date(date('{start_date}') - interval '1' year)
        GROUP BY ss_l.brand_name, ss_l.{zone}, ss_l.member_newold_type, ss_l.date
    ), ss_now AS (
        SELECT DISTINCT f.brand_name, f.{zone}, f.member_newold_type, f.date,
        cast(COALESCE(TRY(sum(f.sales_income) * 1.0 / ss_lyst.sales_income), 0) AS DECIMAL(18, 4)) AS compared_with_ss_lyst
        FROM ads_crm.member_analyse_fold_daily_income_detail f
        LEFT JOIN ss_lyst ON f.brand_name = ss_lyst.brand_name AND f.{zone} = ss_lyst.{zone} AND f.member_newold_type = ss_lyst.member_newold_type
        ...
        AND f.date <= date('{end_date}')
        AND f.date >= date('{start_date}')
        GROUP BY f.brand_name, f.{zone}, f.member_newold_type, ss_lyst.sales_income, f.date
    ), d AS (
        SELECT DISTINCT
            f.brand_name         AS brand,
            array[f.{zone}]      AS zone,
        ...
        AND f.member_newold_type = ss_now.member_newold_type AND f.date = ss_now.date
        WHERE f.member_newold_type IS NOT NULL AND f.member_type IS NULL AND f.member_level_type IS NULL
        AND f.brand_name IN ({brands})
        AND f.{zone} IN ({zones})
        AND f.order_channel IN ({order_channels})
        ...
        AND f.date <= DATE('{end_date}')
        AND f.date >= DATE('{start_date}')
        GROUP BY f.brand_name, f.{zone}, f.member_newold_type, tt.sales_income, lyst.sales_income, ss_now.compared_with_ss_lyst, f.date
    )
    ...
    l.date
    FROM l LEFT JOIN d ON l.brand = d.brand AND l.zone = d.zone AND l.member_type = d.member_type AND l.date = d.date
"""
...
MONTHLY = """
    WITH l AS (
        ...
        SELECT brand_name, {zone}, member_newold_type, year(date) AS year, month(date) AS month,
        cast(sum(sales_income) AS DECIMAL(18, 3)) AS sales_income,
        array_distinct(array_agg(store_code)) AS store_array
        FROM ads_crm.member_analyse_fold_daily_income_detail
        WHERE member_newold_type IS NOT NULL AND member_type IS NULL AND member_level_type IS NULL
        ...
        AND date <= date(date('{end_date}') - interval '1' year)
        AND date >= date(date('{start_date}') - interval '1' year)
        GROUP BY brand_name, {zone}, member_newold_type, year(date), month(date)
    ), ss AS (
        SELECT ss.brand_name, ss.{zone}, ss.member_newold_type, year(ss.date) AS year, month(ss.date) AS month,
        array_intersect(array_distinct(array_agg(ss.store_code)), lyst.store_array) AS store_array
        FROM ads_crm.member_analyse_fold_daily_income_detail ss
        LEFT JOIN lyst ON ss.brand_name = lyst.brand_name AND ss.{zone} = lyst.{zone} AND ss.member_newold_type = lyst.member_newold_type
        AND year(ss.date) - 1 = lyst.year AND month(ss.date) = lyst.month
        WHERE ss.member_newold_type IS NOT NULL AND ss.member_level_type IS NULL AND ss.member_type IS NULL
        AND ss.brand_name IN ({brands})
        AND ss.{zone} IN ({zones})
        AND ss.order_channel IN ({order_channels})
        AND ss.sales_mode IN ({sales_modes})
        ...
        AND ss.date <= date('{end_date}')
        AND ss.date >= date('{start_date}')
        GROUP BY ss.brand_name, ss.{zone}, ss.member_newold_type, lyst.store_array, year(ss.date), month(ss.date)
    ), ss_lyst AS (
        SELECT ss_l.brand_name, ss_l.{zone}, ss_l.member_newold_type, year(ss_l.date) AS year, month(ss_l.date) AS month,
        cast(sum(ss_l.sales_income) AS DECIMAL(18, 3)) AS sales_income
        FROM ads_crm.member_analyse_fold_daily_income_detail ss_l
        LEFT JOIN ss ON ss_l.brand_name = ss.brand_name AND ss_l.{zone} = ss.{zone} AND ss_l.member_newold_type = ss.member_newold_type
        AND ss.year - 1 = year(ss_l.date) AND ss.month = month(ss_l.date)
        WHERE ss_l.member_newold_type IS NOT NULL AND ss_l.member_level_type IS NULL AND ss_l.member_type IS NULL
        AND contains(ss.store_array, ss_l.store_code)
        AND ss_l.brand_name IN ({brands})
        AND ss_l.{zone} IN ({zones})
        AND ss_l.order_channel IN ({order_channels})
        ...
        AND ss_l.date <= date(date('{end_date}') - interval '1' year)
        AND ss_l.date >= date(date('{start_date}') - interval '1' year)
        GROUP BY ss_l.brand_name, ss_l.{zone}, ss_l.member_newold_type, year(ss_l.date), month(ss_l.date)
    ), ss_now AS (
        SELECT DISTINCT f.brand_name, f.{zone}, f.member_newold_type, year(f.date) AS year, month(f.date) AS month,
        cast(COALESCE(TRY(sum(f.sales_income) * 1.0 / ss_lyst.sales_income), 0) AS DECIMAL(18, 4)) AS compared_with_ss_lyst
        FROM ads_crm.member_analyse_fold_daily_income_detail f
        LEFT JOIN ss_lyst ON f.brand_name = ss_lyst.brand_name AND f.{zone} = ss_lyst.{zone} AND f.member_newold_type = ss_lyst.member_newold_type
        ...
        WHERE f.member_newold_type IS NOT NULL AND f.member_level_type IS NULL AND f.member_type IS NULL
        AND contains(ss.store_array, f.store_code)
        AND f.brand_name IN ({brands})
        AND f.{zone} IN ({zones})
        AND f.order_channel IN ({order_channels})
        ...
        AND f.date <= date('{end_date}')
        AND f.date >= date('{start_date}')
        GROUP BY f.brand_name, f.{zone}, f.member_newold_type, ss_lyst.sales_income, year(f.date), month(f.date)
    ), d AS (
        SELECT DISTINCT
            f.brand_name         AS brand,
            array[f.{zone}]      AS zone,
        ...
        AND f.date <= DATE('{end_date}')
        AND f.date >= DATE('{start_date}')
        GROUP BY f.brand_name, f.{zone}, f.member_newold_type, tt.sales_income, lyst.sales_income, ss_now.compared_with_ss_lyst, YEAR(f.date), MONTH(f.date)
    )
    SELECT DISTINCT l.brand, l.zone, l.member_type,
    COALESCE(d.sales_income, 0) AS sales_income,
    COALESCE(d.sales_income_proportion, 0) AS sales_income_proportion,
    ...
    COALESCE(d.compared_with_ss_lyst, 0) AS compared_with_ss_lyst,
    l.year, l.month
    FROM l LEFT JOIN d ON l.brand = d.brand AND l.zone = d.zone AND l.member_type = d.member_type
