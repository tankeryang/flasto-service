ALL = """
    ...
    GROUP BY f.brand_name, f.member_newold_type, tt.sales_income, lyst.sales_income
"""
...
DAILY = """
    WITH l AS (
        SELECT DISTINCT
            a.brand, array_distinct(array_agg(a.zone)) AS zone, a.member_type, b.date
        FROM (
            SELECT DISTINCT
                brand_name AS brand,
                {zone} AS zone,
        ...
        GROUP BY a.brand, a.member_type, b.date
    ), tt AS (
        SELECT
            brand_name,
            cast(sum(sales_income) AS DECIMAL(18, 3)) AS sales_income,
        ...
        WHERE f.member_type IS NULL AND f.member_newold_type IS NOT NULL AND f.member_level_type IS NULL
            ...
            AND f.vchr_date <= '{end_date}'
            AND f.vchr_date >= '{start_date}'
        GROUP BY f.brand_name, f.member_newold_type, tt.sales_income, lyst.sales_income, f.date
    )
    SELECT DISTINCT
        l.brand,
        l.zone AS zone,
...
MONTHLY = """
    WITH l AS (
        SELECT DISTINCT
            a.brand, array_distinct(array_agg(a.zone)) AS zone, a.member_type, b.year_month
        FROM (
            SELECT DISTINCT
                brand_name AS brand,
                {zone} AS zone,
        ...
        GROUP BY a.brand, a.member_type, b.year_month
    ), tt AS (
        SELECT
            brand_name,
            cast(sum(sales_income) AS DECIMAL(18, 3)) AS sales_income,
        ...
        WHERE member_type IS NULL AND member_newold_type IS NOT NULL AND member_level_type IS NULL
            ...
            AND vchr_date <= cast(date('{end_date}') - interval '1' year AS VARCHAR)
            AND vchr_date >= cast(date('{start_date}') - interval '1' year AS VARCHAR)
        GROUP BY brand_name, member_newold_type, year_month
    ), tmp AS (
        SELECT DISTINCT
            f.brand_name AS brand,
            array_distinct(array_agg(f.store_code)) AS zone,
        ...
        WHERE f.member_type IS NULL AND f.member_newold_type IS NOT NULL AND f.member_level_type IS NULL
            ...
            AND f.vchr_date <= '{end_date}'
            AND f.vchr_date >= '{start_date}'
        GROUP BY f.brand_name, f.member_newold_type, tt.sales_income, lyst.sales_income, f.year_month
    )
    SELECT DISTINCT
        l.brand,
        l.zone AS zone,
