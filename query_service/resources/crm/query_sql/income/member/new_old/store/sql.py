ALL = """
    WITH tt AS (
        ...
        SELECT brand_name, member_newold_type,
        ...
        GROUP BY brand_name, member_newold_type
    ), ss AS (
        SELECT ss.brand_name, ss.member_newold_type,
        ...
        LEFT JOIN lyst ON ss.brand_name = lyst.brand_name AND ss.member_newold_type = lyst.member_newold_type
        ...
        GROUP BY ss.brand_name, ss.member_newold_type, lyst.store_array
    ), ss_lyst AS (
        SELECT ss_l.brand_name, ss_l.member_newold_type,
        cast(sum(ss_l.sales_income) AS DECIMAL(18, 3)) AS sales_income
        FROM ads_crm.member_analyse_fold_daily_income_detail ss_l
        LEFT JOIN ss ON ss_l.brand_name = ss.brand_name AND ss_l.member_newold_type = ss.member_newold_type
        ...
        GROUP BY ss_l.brand_name, ss_l.member_newold_type
    ), ss_now AS (
        SELECT DISTINCT f.brand_name, f.member_newold_type,
        cast(COALESCE(TRY(sum(f.sales_income) * 1.0 / ss_lyst.sales_income), 0) AS DECIMAL(18, 4)) AS compared_with_ss_lyst
        FROM ads_crm.member_analyse_fold_daily_income_detail f
        LEFT JOIN ss_lyst ON f.brand_name = ss_lyst.brand_name AND f.member_newold_type = ss_lyst.member_newold_type
        LEFT JOIN ss ON f.brand_name = ss.brand_name AND f.member_newold_type = ss.member_newold_type
        ...
        GROUP BY f.brand_name, f.member_newold_type, ss_lyst.sales_income
    )
    SELECT DISTINCT
    ...
    LEFT JOIN lyst ON f.brand_name = lyst.brand_name AND f.member_newold_type = lyst.member_newold_type
    LEFT JOIN ss_now ON f.brand_name = ss_now.brand_name AND f.member_newold_type = ss_now.member_newold_type
    ...
    AND f.date <= date('{end_date}')
    AND f.date >= date('{start_date}')
    GROUP BY f.brand_name, f.member_newold_type, tt.sales_income, lyst.sales_income, ss_now.compared_with_ss_lyst
"""
...
DAILY = """
    WITH l AS (
        SELECT DISTINCT a.brand_name AS brand, array_distinct(array_agg(a.store_code)) AS zone, a.member_newold_type AS member_type, b.date
        FROM (
            SELECT DISTINCT brand_name, member_newold_type, store_code, 'key' AS key FROM cdm_crm.order_info_detail
            WHERE brand_name IN ({brands}) AND member_newold_type IS NOT NULL AND store_code IN ({zones})
        ) a FULL JOIN (
        ...
        GROUP BY a.brand_name, a.member_newold_type, b.date
    ), tt AS (
        ...
        SELECT brand_name, member_newold_type, date,
        cast(sum(sales_income) AS DECIMAL(18, 3)) AS sales_income,
        array_distinct(array_agg(store_code)) AS store_array
        FROM ads_crm.member_analyse_fold_daily_income_detail
        WHERE member_newold_type IS NOT NULL AND member_type IS NULL AND member_level_type IS NULL
        ...
        AND date <= date(date('{end_date}') - interval '1' year)
        AND date >= date(date('{start_date}') - interval '1' year)
        GROUP BY brand_name, member_newold_type, date
    ), ss AS (
        SELECT ss.brand_name, ss.member_newold_type, ss.date,
        array_intersect(array_distinct(array_agg(ss.store_code)), lyst.store_array) AS store_array
        FROM ads_crm.member_analyse_fold_daily_income_detail ss
        LEFT JOIN lyst ON ss.brand_name = lyst.brand_name AND ss.member_newold_type = lyst.member_newold_type
        ...
        AND ss.date <= date('{end_date}')
        AND ss.date >= date('{start_date}')
        GROUP BY ss.brand_name, ss.member_newold_type, lyst.store_array, ss.date
    ), ss_lyst AS (
        SELECT ss_l.brand_name, ss_l.member_newold_type, ss_l.date,
        cast(sum(ss_l.sales_income) AS DECIMAL(18, 3)) AS sales_income
        FROM ads_crm.member_analyse_fold_daily_income_detail ss_l
        LEFT JOIN ss ON ss_l.brand_name = ss.brand_name AND ss_l.member_newold_type = ss.member_newold_type
        ...
        AND ss_l.date <= date(date('{end_date}') - interval '1' year)
        AND ss_l.date >= date(date('{start_date}') - interval '1' year)
        GROUP BY ss_l.brand_name, ss_l.member_newold_type, ss_l.date
    ), ss_now AS (
        SELECT DISTINCT f.brand_name, f.member_newold_type, f.date,
        cast(COALESCE(TRY(sum(f.sales_income) * 1.0 / ss_lyst.sales_income), 0) AS DECIMAL(18, 4)) AS compared_with_ss_lyst
        FROM ads_crm.member_analyse_fold_daily_income_detail f
        LEFT JOIN ss_lyst ON f.brand_name = ss_lyst.brand_name AND f.member_newold_type = ss_lyst.member_newold_type
        ...
        AND f.date <= date('{end_date}')
        AND f.date >= date('{start_date}')
        GROUP BY f.brand_name, f.member_newold_type, ss_lyst.sales_income, f.date
    ), d AS (
        SELECT DISTINCT
            f.brand_name         AS brand,
            array_distinct(array_agg(f.store_code)) AS zone,
        ...
        AND f.date <= DATE('{end_date}')
        AND f.date >= DATE('{start_date}')
        GROUP BY f.brand_name, f.member_newold_type, tt.sales_income, lyst.sales_income, ss_now.compared_with_ss_lyst, f.date
    )
...
MONTHLY = """
    WITH l AS (
        SELECT DISTINCT a.brand_name AS brand, array_distinct(array_agg(a.store_code)) AS zone, a.member_newold_type AS member_type,
        b.year, b.month
        FROM (
            SELECT DISTINCT brand_name, member_newold_type, store_code, 'key' AS key FROM cdm_crm.order_info_detail
            WHERE brand_name IN ({brands}) AND member_newold_type IS NOT NULL AND store_code IN ({zones})
        ...
        GROUP BY a.brand_name, a.member_newold_type, b.year, b.month
    ), tt AS (
        ...
        SELECT brand_name, member_newold_type, year(date) AS year, month(date) AS month,
        cast(sum(sales_income) AS DECIMAL(18, 3)) AS sales_income,
        array_distinct(array_agg(store_code)) AS store_array
        FROM ads_crm.member_analyse_fold_daily_income_detail
        WHERE member_newold_type IS NOT NULL AND member_type IS NULL AND member_level_type IS NULL
        ...
        AND date <= date(date('{end_date}') - interval '1' year)
        AND date >= date(date('{start_date}') - interval '1' year)
        GROUP BY brand_name, member_newold_type, year(date), month(date)
    ), ss AS (
        SELECT ss.brand_name, ss.member_newold_type, year(ss.date) AS year, month(ss.date) AS month,
        array_intersect(array_distinct(array_agg(ss.store_code)), lyst.store_array) AS store_array
        FROM ads_crm.member_analyse_fold_daily_income_detail ss
        LEFT JOIN lyst ON ss.brand_name = lyst.brand_name AND ss.member_newold_type = lyst.member_newold_type
        ...
        AND ss.date <= date('{end_date}')
        AND ss.date >= date('{start_date}')
        GROUP BY ss.brand_name, ss.member_newold_type, lyst.store_array, year(ss.date), month(ss.date)
    ), ss_lyst AS (
        SELECT ss_l.brand_name, ss_l.member_newold_type, year(ss_l.date) AS year, month(ss_l.date) AS month,
        cast(sum(ss_l.sales_income) AS DECIMAL(18, 3)) AS sales_income
        FROM ads_crm.member_analyse_fold_daily_income_detail ss_l
        LEFT JOIN ss ON ss_l.brand_name = ss.brand_name AND ss_l.member_newold_type = ss.member_newold_type
        ...
        AND ss_l.date <= date(date('{end_date}') - interval '1' year)
        AND ss_l.date >= date(date('{start_date}') - interval '1' year)
        GROUP BY ss_l.brand_name, ss_l.member_newold_type, year(ss_l.date), month(ss_l.date)
    ), ss_now AS (
        SELECT DISTINCT f.brand_name, f.member_newold_type, year(f.date) AS year, month(f.date) AS month,
        cast(COALESCE(TRY(sum(f.sales_income) * 1.0 / ss_lyst.sales_income), 0) AS DECIMAL(18, 4)) AS compared_with_ss_lyst
        FROM ads_crm.member_analyse_fold_daily_income_detail f
        LEFT JOIN ss_lyst ON f.brand_name = ss_lyst.brand_name AND f.member_newold_type = ss_lyst.member_newold_type
        ...
        AND f.date <= date('{end_date}')
        AND f.date >= date('{start_date}')
        GROUP BY f.brand_name, f.member_newold_type, ss_lyst.sales_income, year(f.date), month(f.date)
    ), d AS (
        SELECT DISTINCT
            f.brand_name         AS brand,
            array_distinct(array_agg(f.store_code)) AS zone,
        ...
        AND f.date <= DATE('{end_date}')
        AND f.date >= DATE('{start_date}')
        GROUP BY f.brand_name, f.member_newold_type, tt.sales_income, lyst.sales_income, ss_now.compared_with_ss_lyst, YEAR(f.date), MONTH(f.date)
    )
    SELECT DISTINCT l.brand, l.zone, l.member_type,
    COALESCE(d.sales_income, 0) AS sales_income,
    COALESCE(d.sales_income_proportion, 0) AS sales_income_proportion,
