...
DAILY = """
    WITH tt AS (
        ...
        SELECT brand_name, store_code, member_type, cast(sum(sales_income) AS DECIMAL(18, 3)) AS sales_income, date
        FROM ads_crm.member_analyse_fold_daily_income_detail
        WHERE member_type IS NOT NULL AND member_newold_type IS NULL AND member_level_type IS NULL
        AND brand_name IN ({brands})
        AND store_code IN ({zones})
        ...
        AND date <= date(date('{end_date}') - interval '1' year)
        AND date >= date(date('{start_date}') - interval '1' year)
        GROUP BY brand_name, store_code, member_type, date
    )
    SELECT DISTINCT
        f.brand_name AS brand,
        f.store_code AS zone,
    ...
    AND f.date <= date('{end_date}')
    AND f.date >= date('{start_date}')
    GROUP BY f.brand_name, f.store_code, f.member_type, tt.sales_income, lyst.sales_income, f.date
"""
...
MONTHLY = """
    WITH tt AS (
        ...
        AND date <= date(date('{end_date}') - interval '1' year)
        AND date >= date(date('{start_date}') - interval '1' year)
        GROUP BY brand_name, store_code, member_type, year(date), month(date)
    )
    SELECT DISTINCT
        f.brand_name AS brand,
        f.store_code AS zone,
    ...
    AND f.date <= date('{end_date}')
    AND f.date >= date('{start_date}')
    GROUP BY f.brand_name, f.store_code, f.member_type, tt.sales_income, lyst.sales_income, year(f.date), month(f.date)
"""
