ALL = """
    ...
    AND f.store_level IN ({store_levels})
    AND f.channel_type IN ({channel_types})
    AND date(f.order_deal_time) >= date('{start_date}')
    AND date(f.order_deal_time) <= date('{end_date}')
...
DAILY = """
    WITH l AS (
        ...
        SELECT DISTINCT brand_name, {zone}, member_no, date(member_register_time) AS date
        FROM cdm_crm.member_info_detail
        WHERE brand_name IN ({brands})
        AND {zone} IN ({zones})
        AND sales_mode IN ({sales_modes})
        ...
        SELECT brand_name, {zone}, count(DISTINCT member_no) AS register_member_amount, date
        FROM tt GROUP BY brand_name, {zone}, date
    ), tt_lyst AS (
        SELECT DISTINCT brand_name, {zone}, member_no, date(member_register_time) AS date
        FROM cdm_crm.member_info_detail
        WHERE brand_name IN ({brands})
        ...
        SELECT brand_name, {zone}, count(DISTINCT member_no) AS register_member_amount, date
        FROM tt_lyst GROUP BY brand_name, {zone}, date
    ), cs_lyst AS (
        SELECT DISTINCT
            f.brand_name,
            f.{zone},
        ...
        INNER JOIN cs_lyst ON f.brand_name = cs_lyst.brand_name AND f.{zone} = cs_lyst.{zone} AND f.order_deal_date - interval '1' year = cs_lyst.date
        WHERE f.member_type = '会员'
        AND f.brand_name IN ({brands})
        AND f.order_channel IN ({order_channels})
        AND f.{zone} IN ({zones})
        ...
        AND date(f.order_deal_time) >= date('{start_date}')
        GROUP BY f.brand_name, f.{zone}, tt_num.register_member_amount, tt_lyst_num.register_member_amount,
            cs_lyst.consumed_member_amount, cs_lyst.unconsumed_member_amount, f.order_deal_date
    )
...
MONTHLY = """
    WITH l AS (
        ...
        SELECT DISTINCT brand_name, {zone}, member_no, substr(cast(date(member_register_time) AS VARCHAR), 1, 7) AS year_month
        FROM cdm_crm.member_info_detail
        WHERE brand_name IN ({brands})
        AND {zone} IN ({zones})
        AND sales_mode IN ({sales_modes})
        ...
        SELECT brand_name, {zone}, count(DISTINCT member_no) AS register_member_amount, year_month
        FROM tt GROUP BY brand_name, {zone}, year_month
    ), tt_lyst AS (
        SELECT DISTINCT brand_name, {zone}, member_no, substr(cast(date(member_register_time) AS VARCHAR), 1, 7) AS year_month
        FROM cdm_crm.member_info_detail
        WHERE brand_name IN ({brands})
        ...
        SELECT brand_name, {zone}, count(DISTINCT member_no) AS register_member_amount, year_month
        FROM tt_lyst GROUP BY brand_name, {zone}, year_month
    ), cs_lyst AS (
        SELECT DISTINCT
            f.brand_name,
            f.{zone},
            cast(count(DISTINCT f.member_no) AS INTEGER) AS consumed_member_amount,
            cast(tt_lyst_num.register_member_amount - count(DISTINCT f.member_no) AS INTEGER) AS unconsumed_member_amount,
            substr(cast(f.order_deal_date AS VARCHAR), 1, 7) AS year_month
        FROM cdm_crm.order_info_detail f
        INNER JOIN tt_lyst_num ON f.brand_name = tt_lyst_num.brand_name AND f.{zone} = tt_lyst_num.{zone}
            AND substr(cast(f.order_deal_date AS VARCHAR), 1, 7) = tt_lyst_num.year_month
        WHERE f.member_type = '会员'
        AND f.brand_name IN ({brands})
        AND f.order_channel IN ({order_channels})
        AND f.{zone} IN ({zones})
        ...
        SELECT DISTINCT
            ...
            substr(cast(f.order_deal_date AS VARCHAR), 1, 7) AS year_month
        FROM cdm_crm.order_info_detail f
        INNER JOIN tt ON f.brand_name = tt.brand_name AND f.{zone} = tt.{zone} AND f.member_no = tt.member_no AND substr(cast(f.order_deal_date AS VARCHAR), 1, 7) = tt.year_month
        INNER JOIN tt_num ON f.brand_name = tt_num.brand_name AND f.{zone} = tt_num.{zone} AND substr(cast(f.order_deal_date AS VARCHAR), 1, 7) = tt_num.year_month
        INNER JOIN tt_lyst_num ON f.brand_name = tt_lyst_num.brand_name
            AND f.{zone} = tt_lyst_num.{zone}
        ...
        AND date(f.order_deal_time) >= date('{start_date}')
        GROUP BY f.brand_name, f.{zone}, tt_num.register_member_amount, tt_lyst_num.register_member_amount,
            cs_lyst.consumed_member_amount, cs_lyst.unconsumed_member_amount, substr(cast(f.order_deal_date AS VARCHAR), 1, 7)
    )
    SELECT DISTINCT l.brand, l.zone,
    COALESCE(d.register_member_amount, 0) AS register_member_amount,
    COALESCE(d.rma_compared_with_lyst, 0) AS rma_compared_with_lyst,
