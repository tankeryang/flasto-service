...
MONTHLY = """
    WITH l AS (
        SELECT DISTINCT a.brand_name AS brand, array[a.{zone}] AS zone, a.member_register_type, b.year, b.month
        FROM (
            SELECT DISTINCT brand_name, member_register_type, {zone}, 'key' AS key
            FROM ads_crm.member_recruit_analyse_fold_daily_income_detail
            WHERE brand_name IN ({brands}) AND member_register_type IS NOT NULL AND {zone} IN ({zones})
        ...
        AND f.date <= DATE('{end_date}') - INTERVAL '1' DAY
        AND f.date >= DATE('{start_date}')
        GROUP BY f.brand_name, f.{zone}, f.member_register_type, year(f.date), month(f.date), tt.register_member_array, tt.register_member_amount
    )
    SELECT DISTINCT l.brand, l.zone, l.member_register_type,
    COALESCE(d.member_amount, 0) AS member_amount,
    COALESCE(d.member_amount_proportion, 0) AS member_amount_proportion,
    l.year, l.month
    FROM l LEFT JOIN d ON l.brand = d.brand AND l.zone = d.zone AND l.member_register_type = d.member_register_type
