...
def member_analyse_formator(sql, payload):
    """
    payload 参数校验
    sql 参数填充
    :param sql: sql字符串
    ...
    if 'store_codes' not in payload.keys():
        # 校验参数
        necessary_param = dto.member_anlyse.zone.ZONE_DTO.keys()
        ...
        # 校验参数
        necessary_param = dto.member_anlyse.store.STORE_DTO.keys()
        if not validator(necessary_param, payload):
            return None
...
def daily_report_formator(sql, payload):
    ...
    # 校验参数
    necessary_param = dto.report_center.DAILY_DTO.keys()
