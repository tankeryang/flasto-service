...
income_analyse_zone_qo = ns.model('IncomeAnalyseQO', {
    ...
    'trade_source': fields.List(fields.String(
        description="订单渠道(FPOS/IPOS/OMIS/官网/其他)", example='FPOS', enum=['全部', 'FPOS', 'IPOS', 'OMIS', '官网', '其他']
    ), required=True),
    'start_date': fields.Date(description="开始日期(yyyy-mm-dd)", example='2018-11-01', required=True),
    'end_date': fields.Date(description="结束日期(yyyy-mm-dd)", example='2018-11-30', required=True)
})


income_analyse_store_qo = ns.model('IncomeAnalyseStoreQO', {
    'brands': fields.List(fields.String(description="品牌名", example="FivePlus"), required=True),
