...
crm_member_analyse_req_dto_model = ns_1.model('CrmMemberAnalyseReqDtoModel', {
    'sales_modes': fields.List(fields.String(description="门店类别", enum=['正价', '长特', '短特']), required=True),
    'store_types': fields.List(fields.String(description="门店类型", enum=['MALL', '百货', '专卖店']), required=True),
    'store_levels': fields.List(fields.String(description="门店等级", enum=['A', 'B', 'C']), required=True),
    'channel_types': fields.List(fields.String(description="经营方式/渠道", enum=['自营', '联营', '特许']), required=True),
    'order_channels': fields.List(fields.String(description="订单类型(线上/线下)", enum=['线下', '线上']), required=True),
